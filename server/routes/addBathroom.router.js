...
router.post('/add', rejectUnauthenticated, async (req, res) => {
  ...
  try {
    ...
    const restroomsQuery = `
          ...
          RETURNING "id"
          `;
    const restroomsValues = [info.name, street, city, state, info.accessible, info.unisex, info.latitude, info.longitude, country, info.changing_table, info.single_stall, info.placeID]
